#include "education_details.h" // Our new logic header

// --- MAIN CGI PROGRAM ---
int main(void) {
    struct EducationNode *list_head = NULL;
    char *action = NULL;
    ...
    list_head = load_education_data();

    if (action && strcmp(action, "add") == 0) {
        cJSON *data = cJSON_GetObjectItem(request_json, "data");
        char *name = cJSON_GetObjectItem(data, "name")->valuestring;
        char *course_year = cJSON_GetObjectItem(data, "course_year")->valuestring;
        float score = (float)cJSON_GetObjectItem(data, "score")->valuedouble;
        int index = cJSON_GetObjectItem(data, "index")->valueint;
        
        list_head = insert_education_at_index(list_head, name, course_year, score, index);
        save_education_data(list_head);
        ...
        list_head = delete_education_at_index(list_head, index);
        save_education_data(list_head);
        response_json = cJSON_CreateObject();
        cJSON_AddStringToObject(response_json, "status", "success");
        
    } else if (action && strcmp(action, "preview") == 0) {
        response_json = serialize_education_data(list_head);
    }
