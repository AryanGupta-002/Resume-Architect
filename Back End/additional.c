#include "additional_details.h" // Our new logic header

// --- MAIN CGI PROGRAM ---
int main(void) {
    struct AdditionalNode *list_head = NULL;
    char *action = NULL;
    ...
    list_head = load_additional_data();

    if (action && strcmp(action, "add") == 0) {
        cJSON *data = cJSON_GetObjectItem(request_json, "data");
        char *detail = cJSON_GetObjectItem(data, "detail")->valuestring;
        int index = cJSON_GetObjectItem(data, "index")->valueint;
        
        list_head = insert_additional_at_index(list_head, detail, index);
        save_additional_data(list_head);
        ...
        list_head = delete_additional_at_index(list_head, index);
        save_additional_data(list_head);
        response_json = cJSON_CreateObject();
        cJSON_AddStringToObject(response_json, "status", "success");
        
    } else if (action && strcmp(action, "preview") == 0) {
        response_json = serialize_additional_data(list_head);
    }
    ...
    free_additional_list(list_head);
